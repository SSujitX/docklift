...
export const CustomDomains = () => (
  <section id="domains" className="scroll-mt-20 mb-12 text-left">
    ...
    <p className="text-muted-foreground mb-4">
      ...
      Docklift issues free Let&apos;s Encrypt certificates automatically (ports 80 + 443).
    </p>
    ...
    <div className="bg-secondary/50 rounded-xl p-6 mb-6">
      ...
      <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
        <li>Point a DNS <strong>A record</strong> at your server IP</li>
        <li>Go to <strong>Settings → Domain</strong></li>
        <li>Optionally set the ACME email, then add your domain (e.g., <code className="bg-primary/10 px-1.5 py-0.5 rounded text-primary">panel.example.com</code>)</li>
        <li>Wait until SSL shows <strong>HTTPS Active</strong> (Retry if DNS was not ready)</li>
      </ol>
    </div>
    ...
    <div className="bg-secondary/50 rounded-xl p-6 mb-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Assign custom domains to individual project services (each Dockerfile/service can have multiple domains).
      </p>
      <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
    ...
    <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-xl p-4 border border-cyan-500/20 space-y-2">
      <p className="text-sm">
        <span className="font-semibold text-cyan-500">Cloudflare:</span>{" "}
        After SSL is <strong>Active</strong> in Docklift, set SSL/TLS mode to{" "}
        <strong>Full (strict)</strong>. Prefer DNS-only (gray cloud) while the certificate is first issuing,
        then enable the orange cloud if you want Cloudflare proxy.
      </p>
      <p className="text-sm text-muted-foreground">
        Do <strong>not</strong> use Flexible — that sends HTTP to your origin and is weaker than Full (strict).
      </p>
    </div>
  </section>
);
