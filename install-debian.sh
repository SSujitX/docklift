...
echo -e "${BOLD}🚀 Starting Debian Installation...${NC}\n"

# Step 1: Dependencies
printf "${CYAN}[1/5]${NC} Installing Debian dependencies..."
{
    apt-get update -qq && apt-get install -y -qq curl sudo procps git >/dev/null 2>&1
} &
spinner $!

# Step 2: Docker
if ! command -v docker &> /dev/null; then
    printf "${CYAN}[2/5]${NC} Installing Docker..."
    if [ "$CI" = "true" ]; then
